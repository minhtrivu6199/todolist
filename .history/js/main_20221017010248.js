...
function completeToDo(id, e) {
    ...
    if (status == "completed") {
        var keyComplete = todoList.findIndex(id);
        var completedItem = todoList.arr.slice(keyComplete, keyComplete+1);
        var objToDo = new task(completedItem[0].taskName,"todo");
        moveToDo(todoList,completeList,objToDo, keyComplete);
        showToDoList();
        showCompleteList();
    }
}
